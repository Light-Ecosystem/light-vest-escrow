...
contract LightTeamVaultManagerUpgradeV2 is OwnableUpgradeable {
    ...
    modifier onlyOwnerOrOperator() {
        require(owner() == _msgSender() || operators[_msgSender()], "LightTeamVaultManager: caller is not the owner or operator");
        _;
    }
     ...
     */
    function claimUnlockedLTAndLockForVeLT() external onlyOwnerOrOperator returns (uint256) {
     ...
     */
    function voteForGaugesWeights(address[] calldata gaugeAddresses, uint256[] calldata userWeights) external onlyOwnerOrOperator {
    ...
    function revokeOperator(address _operator) external onlyOwner {
        ...
        emit SetOperator(_operator, false);
    }
