...
describe("LightTeamVaultManagerUpgrade", function () {
    async function fixture() {
        ...
        const VaultManager = await ethers.getContractFactory("LightTeamVaultManagerUpgrade");
    ...
    describe("claimUnlockedLTAndLockForVeLT", async () => {
        it("should be revert if caller is not owner", async () => {
            ...
            await expect(vaultManager.connect(alice).claimUnlockedLTAndLockForVeLT())
                .to.be.revertedWith("LightTeamVaultManager: caller is not the owner or operator");
        });

        it("should be success if caller is operator", async () => {
            const { alice, vaultManager, veLT } = await loadFixture(fixture);
            await expect(vaultManager.grantOperator(alice.address)).to.be.emit(vaultManager, "SetOperator").withArgs(alice.address, true);
            const claimTime = (await time.latest()) + ONE_DAY;
            await time.increaseTo(claimTime);
            await vaultManager.connect(alice).claimUnlockedLTAndLockForVeLT();
        ...
        it("should be revert if operator role revoke", async () => {
            const { alice, vaultManager, veLT } = await loadFixture(fixture);
            await vaultManager.grantOperator(alice.address);
            await expect(vaultManager.revokeOperator(alice.address)).to.be.emit(vaultManager, "SetOperator").withArgs(alice.address, false);
            await expect(vaultManager.connect(alice).claimUnlockedLTAndLockForVeLT())
                .to.be.revertedWith("LightTeamVaultManager: caller is not the owner or operator");
        });
